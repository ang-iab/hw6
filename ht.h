...
bool HashTable<K,V,Prober,Hash,KEqual>::empty() const
{
    for (size_t i = 0; i < table_.size(); ++i)
    {
        if (table_[i] != nullptr)
        {
            if (table_[i]->deleted == false) return false;
        }
    }
...
size_t HashTable<K,V,Prober,Hash,KEqual>::size() const
{
    ...
    for (size_t i = 0; i < CAPACITIES[mIndex_]; ++i)
    {
        if (table_[i] != nullptr) 
        {
            if (table_[i]->deleted == false) ++size;
        }
    }
...
{
    if (static_cast<double>(numItems_) / CAPACITIES[mIndex_] >= resizeAlpha_) resize();
    
    HASH_INDEX_T loc = probe(p.first);

    if (loc == npos) throw std::logic_error("No Free Location Can Be Found");
...
void HashTable<K,V,Prober,Hash,KEqual>::remove(const KeyType& key)
{
    for (size_t i = 0; i < CAPACITIES[mIndex_]; ++i)
    {
        if (table_[i] != nullptr)
        {
            if (table_[i]->deleted == false && table_[i]->item.first == key)
            {
...
void HashTable<K,V,Prober,Hash,KEqual>::resize()
{
    ...
    for (HASH_INDEX_T i = 0; i < CAPACITIES[mIndex_ - 1]; ++i)
    {
        if (oldTable[i] != nullptr)
        {
            ...
            {
                HASH_INDEX_T hash = hash_(oldTable[i]->item.first) % CAPACITIES[mIndex_];
                
                prober_.init(hash, CAPACITIES[mIndex_], oldTable[i]->item.first);

                HASH_INDEX_T loc = prober_.next();

                while (loc != npos && table_[loc] != nullptr) loc = prober_.next();

                if (loc == npos) throw std::logic_error("No More CAPACITIES exist");
